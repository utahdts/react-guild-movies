...
export default function useRecordingFetcher(): { data: [IRecording], status: string } {
    const { data, status } = useQuery(
        ...
        () => fetch(`/api/recordings`)
            ...
            .then((recordings: [IRecording]): [IRecording] => {
                ...
                recordings.forEach((video: IRecording) => video.nameLowerCase = video.name.toLowerCase());
